...
interface CompanyEmailRejectModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const CompanyEmailRejectModal: React.FC<CompanyEmailRejectModalProps> = ({
  isOpen,
  onClose,
  onReject,
  companyEmail,
    ...
    >
      <ModalContent>
        ...
        <HeaderSection>
          ...
          <HeaderText>
            <HeaderTitle>회사 이메일 인증 거절</HeaderTitle>
            <HeaderDescription>
              이 회사 이메일 인증을 거절하시겠습니까? 거절 사유는 사용자에게 전달됩니다.
            </HeaderDescription>
          </HeaderText>
        </HeaderSection>
...
const EmailInfoHeader = styled.div`
  display: flex;
  align-items: center;
  gap: 8px;
  font-size: 13px;
...
export default CompanyEmailRejectModal;
